...
const UrlShortener = () => {
  ...
  const handleCreateLink = async (linkData) => {
    try {
      // Try to use API if available, otherwise fallback to local storage
      ...
      if (apiUrl) {
        const response = await fetch(`${apiUrl}/api/urls`, {
          ...
          body: JSON.stringify({
            originalUrl: linkData.destination,
        ...
        if (response.ok) {
          ...
          const formattedUrl = {
            ...
            destinations: [{url: newUrl.originalUrl}],
          ...
          setWorkspaces(prev => ({ ...prev, [currentWorkspaceId]: { ...prev[currentWorkspaceId], urls: [formattedUrl, ...currentUrls] } }));
          return;
        }
      }
